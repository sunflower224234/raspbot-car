...
function commandBlockReason(command, s) {
  ...
  if (["face", "faceFail", "logout", "clearLogs"].includes(command)) return "";
  ...
  if (command === "pause" && !modeIsRunning(s)) return "只有自动巡线中才能暂停";
  if (command === "resume" && !(modeIsObstacle(s) || s.task_status === "paused" || s.task_status === "error_stop" || s.task_status === "obstacle_stop" || s.mode === "READY")) {
    return "当前状态不需要继续任务";
  }
  ...
  const modules = {
    ...
    run: `
      ...
      <div class="action-row">
        ...
        <button class="ghost-btn" data-command="pause">暂停</button>
        <button class="primary-btn" data-command="resume">继续</button>
